...
import static me.nino.anglersdream.fishing.LootManager.getRandomLootBox;
...
public class FishEventHandler {
    ...
    public static void onFish(ItemFishedEvent event) {
        if (event.getEntity() instanceof Player player && player.getMainHandItem().getItem() instanceof FishingRodItem) {
            ...
            stack.set(AnglersDreamComponents.LAST_TIME_FISHED, (int) (new Date().getTime() / 1000L));
            ...
            if (chance(baseChance)) {
                ...
                updateCaughtCount(stack, AnglersDreamComponents.BAGS_CAUGHT);
            }
    ...
    private static void processDrops(List<ItemStack> drops, ItemStack stack) {
        for (ItemStack drop : drops) {
            ...
            switch (type.toLowerCase()) {
                case "junk":
                    updateCaughtCount(stack, AnglersDreamComponents.JUNK_CAUGHT);
                    break;
                case "fish":
                    updateCaughtCount(stack, AnglersDreamComponents.FISH_CAUGHT);
                    break;
                case "treasure":
                    updateCaughtCount(stack, AnglersDreamComponents.TREASURE_CAUGHT);
                    break;
            }
        }
    }
